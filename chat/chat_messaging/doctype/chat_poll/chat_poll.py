...
class ChatPoll(Document):
	...
	if TYPE_CHECKING:
		...
		from chat.chat_messaging.doctype.chat_poll_option.chat_poll_option import ChatPollOption

		is_anonymous: DF.Check
		is_disabled: DF.Check
		is_multi_choice: DF.Check
		options: DF.Table[ChatPollOption]
	...
	def before_validate(self):
		...
		poll_votes = frappe.get_all(
			"Chat Poll Vote", filters={"poll_id": self.name}, fields=["user_id"], group_by="user_id"
		)
	...
	def on_trash(self):
		...
		frappe.db.delete("Chat Poll Vote", {"poll_id": self.name})
