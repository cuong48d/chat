...
from chat.ai.ai import handle_ai_thread_message, handle_bot_dm
from chat.api.chat_channel import get_peer_user
from chat.notification import send_notification_to_topic, send_notification_to_user
from chat.utils import clear_thread_reply_count_cache, get_thread_reply_count, track_channel_visit
...
class ChatMessage(Document):
	...
	if TYPE_CHECKING:
		...
		from chat.chat_messaging.doctype.chat_mention.chat_mention import ChatMention
	...
	def validate_linked_message(self):
		...
		if self.linked_message:
			if (
				frappe.get_cached_value("Chat Message", self.linked_message, "channel_id") != self.channel_id
			):
	...
	def handle_ai_message(self):
		...
		channel_doc = frappe.get_cached_doc("Chat Channel", self.channel_id)
		...
		peer_user_doc = frappe.get_cached_doc("Chat User", peer_user.get("user_id"))

		if peer_user_doc.type != "Bot" or not peer_user_doc.bot:
			return

		bot = frappe.get_cached_doc("Chat Bot", peer_user_doc.bot)
	...
	def set_last_message_timestamp(self):
		...
		chat_channel = frappe.qb.DocType("Chat Channel")
		query = (
			frappe.qb.update(chat_channel)
			.where(chat_channel.name == self.channel_id)
			.set(chat_channel.last_message_timestamp, self.creation)
			.set(chat_channel.last_message_details, json.dumps(message_details))
		)
		query.run()
	...
	def publish_unread_count_event(self):
		...
		if channel_doc.is_direct_message:
			...
			# This event needs to be published to all users on Chat (desk + website)
			frappe.publish_realtime(
				"chat:unread_channel_count_updated",
				{
					"channel_id": self.channel_id,
	...
	def send_push_notification(self):
		...
		if channel_doc.is_direct_message:
			...
			# channel_type = frappe.get_cached_value("Chat Channel", self.channel_id, "channel_type")
	...
	def get_message_owner_name(self):
		...
		"""
		if self.is_bot_message:
			return frappe.get_cached_value("Chat User", self.bot, "full_name")
		else:
			return frappe.get_cached_value("Chat User", self.owner, "full_name")
	...
	def send_notification_for_direct_message(self):
		...
		"""
		peer_chat_user = frappe.db.get_value(
			"Chat Channel Member",
			{"channel_id": self.channel_id, "user_id": ("!=", self.owner)},
			"user_id",
		)

		if not peer_chat_user:
			return

		peer_chat_user_doc = frappe.get_cached_doc("Chat User", peer_chat_user)

		# Do not send notification to a bot
		if peer_chat_user_doc.type == "Bot":
			return
		...
		send_notification_to_user(
			user_id=peer_chat_user_doc.user,
			...
			data={
				"message_id": self.name,
				"channel_id": self.channel_id,
				"chat_message_type": self.message_type,
				"channel_type": "DM",
	...
	def send_notification_for_channel_message(self):
		...
		is_thread = frappe.get_cached_value("Chat Channel", self.channel_id, "is_thread")
		...
		if is_thread:
			...
			channel_name = frappe.get_cached_value("Chat Channel", self.channel_id, "channel_name")
	...
	def send_notification_for_mentions(self, user):
		try:
			...
			push_notification = PushNotification("chat")
	...
	def after_delete(self):
		...
		if self.message_type == "Poll":
			frappe.delete_doc("Chat Poll", self.poll_id)
	...
	def on_trash(self):
		...
		frappe.db.delete("Chat Message Reaction", {"message": self.name})
		...
		if self.is_thread:
			...
			thread_channel_doc = frappe.get_doc("Chat Channel", self.name)
			thread_channel_doc.delete(ignore_permissions=True)

		# delete the pinned message
		is_pinned = frappe.get_all(
			"Chat Pinned Messages", {"message_id": self.name, "parent": self.channel_id}
		)
		if is_pinned:
			channel_doc = frappe.get_doc("Chat Channel", self.channel_id)
			pinned_row = None
...
def on_doctype_update():
	"""
	Add indexes to Chat Message table
	"""
	# Index the selector (channel or message type) first for faster queries (less rows to sort in the next step)
	frappe.db.add_index("Chat Message", ["channel_id", "creation"])
	frappe.db.add_index("Chat Message", ["message_type", "creation"])
