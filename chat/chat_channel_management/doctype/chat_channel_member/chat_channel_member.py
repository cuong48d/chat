...
from chat.notification import subscribe_user_to_topic, unsubscribe_user_to_topic
from chat.utils import delete_channel_members_cache


class ChatChannelMember(Document):
	...
	def before_insert(self):
		...
		if frappe.db.exists(
			"Chat Channel Member", {"channel_id": self.channel_id, "user_id": self.user_id}
		):
		...
		if frappe.db.count("Chat Channel Member", {"channel_id": self.channel_id}) == 0:
			self.is_admin = 1
	...
	def after_delete(self):
		...
		member_name = frappe.get_cached_value("Chat User", self.user_id, "full_name")

		current_user_name = frappe.get_cached_value("Chat User", frappe.session.user, "full_name")
		...
		if (
			frappe.db.count("Chat Channel Member", {"channel_id": self.channel_id}) == 0
			and frappe.db.get_value("Chat Channel", self.channel_id, "type") == "Private"
		):
			frappe.db.set_value("Chat Channel", self.channel_id, "is_archived", 1)

		# If this member was the only admin, then make the next oldest member an admin
		if (
			self.get_admin_count() == 0
			and frappe.db.count("Chat Channel Member", {"channel_id": self.channel_id}) > 0
		):
			...
			frappe.db.set_value("Chat Channel Member", first_member.name, "is_admin", 1)

			first_member_name = frappe.get_cached_value("Chat User", first_member.user_id, "full_name")
	...
	def check_if_user_is_member(self):
		is_member = True
		channel = frappe.db.get_value("Chat Channel", self.channel_id, ["type", "owner"], as_dict=True)
	...
	def after_insert(self):
		...
		is_direct_message = frappe.get_cached_value(
			"Chat Channel", self.channel_id, "is_direct_message"
		)
	...
	def get_admin_count(self):
		return frappe.db.count("Chat Channel Member", {"channel_id": self.channel_id, "is_admin": 1})

	def is_thread(self):
		return frappe.get_cached_value("Chat Channel", self.channel_id, "is_thread")
