...
class ChatChannel(Document):
	...
	if TYPE_CHECKING:
		...
		from chat.chat.doctype.chat_pinned_messages.chat_pinned_messages import ChatPinnedMessages
		...
		pinned_messages: DF.Table[ChatPinnedMessages]
	...
	def on_trash(self):
		...
		frappe.db.delete("Chat Channel Member", {"channel_id": self.name})

		# delete all messages when channel is deleted
		frappe.db.delete("Chat Message", {"channel_id": self.name})

		# delete all reactions when channel is deleted
		frappe.db.delete("Chat Message Reaction", {"channel_id": self.name})

		# Delete the pinned channels
		frappe.db.delete("Chat Pinned Channels", {"channel_id": self.name})
		...
		if self.is_thread and frappe.db.exists("Chat Message", {"name": self.name}):
			message_channel_id = frappe.get_cached_value("Chat Message", self.name, "channel_id")
			frappe.db.set_value("Chat Message", self.name, "is_thread", 0)
	...
	def after_insert(self):
		...
		if not frappe.flags.in_install and not self.flags.do_not_add_member:
			...
			if self.is_direct_message == 1:
				...
				chat_users = self.channel_name.split(" _ ")
				unique_chat_users = list(set(chat_users))
				self.add_members(unique_chat_users)
			else:
	...
	def validate(self):
		...
		if not self.flags.is_created_by_bot:
			if self.type == "Private" or self.type == "Public":
				if (
					...
					and frappe.db.count("Chat Channel Member", {"channel_id": self.name}) <= 1
				):
					pass
				elif frappe.db.exists(
					"Chat Channel Member", {"channel_id": self.name, "user_id": frappe.session.user}
				):
					pass
				elif frappe.session.user == "Administrator":
					pass
		...
		if self.workspace and self.flags.in_insert:
			if frappe.db.exists(
				"Chat Channel", {"channel_name": self.channel_name, "workspace": self.workspace}
			):
				frappe.throw(
	...
	def before_validate(self):
		...
		if not self.is_direct_message and not self.workspace and not self.is_dm_thread:
			workspaces = frappe.get_all("Chat Workspace")
			if len(workspaces) == 1:
	...
	def add_members(self, members, is_admin=0):
		# members is a list of Chat User IDs
