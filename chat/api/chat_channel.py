...
from chat.api.chat_users import get_current_chat_user
from chat.utils import get_channel_members, is_channel_member, track_channel_visit
...
def toggle_pinned_channel(channel_id):
	...
	"""
	chat_user = get_current_chat_user()
	pinned_channels = chat_user.pinned_channels or []

	is_pinned = False
	for pin in pinned_channels:
		if pin.channel_id == channel_id:
			chat_user.remove(pin)
			is_pinned = True
			break

	if not is_pinned:
		chat_user.append("pinned_channels", {"channel_id": channel_id})
...
def leave_channel(channel_id):
	...
	for member in members:
		frappe.delete_doc("Chat Channel Member", member.name)
...
def toggle_pin_message(channel_id, message_id):
	...
	"""
	channel = frappe.get_doc("Chat Channel", channel_id)
	...
	message_exists = frappe.db.get_value("Chat Message", message_id, "channel_id") == channel_id
