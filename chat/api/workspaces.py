...
from chat.utils import delete_channel_members_cache, delete_workspace_members_cache
...
def get_list():
	...
	"""
	if not frappe.db.exists("Chat User", {"user": frappe.session.user}):
		frappe.throw(_("You do not have access to Chat."), frappe.PermissionError)

	workspace = frappe.qb.DocType("Chat Workspace")
	workspace_member = frappe.qb.DocType("Chat Workspace Member")
...
def join_workspace(workspace: str):
	...
	member = frappe.get_doc(
		{"doctype": "Chat Workspace Member", "workspace": workspace, "user": frappe.session.user}
	).insert()
...
def leave_workspace(workspace: str):
	...
	member = frappe.db.exists(
		"Chat Workspace Member", {"workspace": workspace, "user": frappe.session.user}
	)
	if member:
		member_doc = frappe.get_doc("Chat Workspace Member", member)
		member_doc.delete()
...
def can_create_channel(workspace: str):
	...
	"""
	workspace_doc = frappe.get_doc("Chat Workspace", workspace)
...
def fetch_workspace_members(workspace: str):
	...
	"""
	frappe.has_permission("Chat Workspace", doc=workspace, throw=True)
...
def add_workspace_members(workspace: str, members: list):
	...
	"""
	frappe.has_permission("Chat Workspace", doc=workspace, ptype="write", throw=True)
	...
	for member in members:
		member_doc = frappe.get_doc(
			{"doctype": "Chat Workspace Member", "workspace": workspace, "user": member}
		)
...
def update_workspace_members(workspace: str, members: list):
	...
	for member in members:
		try:
			...
			member_id = frappe.db.exists("Chat Workspace Member", {"workspace": workspace, "user": user})

			# If not a member, delete the member
			if not is_member:
				if member_id:
					member_doc = frappe.get_doc("Chat Workspace Member", member_id)
					member_doc.flags.ignore_cache_invalidation = True
	...
	delete_workspace_members_cache(workspace)
	workspace_channels = frappe.get_all("Chat Channel", {"workspace": workspace}, pluck="name")
