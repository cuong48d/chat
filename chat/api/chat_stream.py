...
from chat.utils import track_channel_visit
...
def get_messages_around_base(channel_id: str, base_message: str):
	...
	from_timestamp = frappe.get_cached_value("Chat Message", base_message, "creation")
...
def get_older_messages(channel_id: str, from_message: str, limit: int = 20):
	...
	if not frappe.has_permission(doctype="Chat Channel", doc=channel_id, ptype="read"):
		frappe.throw(_("You do not have permission to access this channel"), frappe.PermissionError)
	# Fetch older messages for the channel
	from_timestamp = frappe.get_cached_value("Chat Message", from_message, "creation")
