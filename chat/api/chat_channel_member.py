...
from chat.utils import delete_channel_members_cache, get_channel_member, track_channel_visit


@frappe.whitelist()
def remove_channel_member(user_id, channel_id):
	# Get chat channel member name where user_id and channel_id match
	member = get_channel_member(channel_id, user_id)
	# Delete chat channel member
	if member:
...
def add_channel_members(channel_id: str, members: list[str]):
	...
	for member in members:
		member_doc = frappe.get_doc(
			{"doctype": "Chat Channel Member", "channel_id": channel_id, "user_id": member}
		)
