...
def toggle_push_notification_for_channel(member: str, allow_notifications: 0 | 1) -> None:
	if are_push_notifications_enabled():
		member_doc = frappe.get_doc("Chat Channel Member", member)
		if member_doc:
...
def subscribe(fcm_token: str, environment: str, device_information: str | None = None) -> None:
	...
	if frappe.db.exists("Chat Push Token", {"fcm_token": fcm_token, "user": frappe.session.user}):
		return

	# Add the FCM token to the database
	frappe.get_doc(
		{
			"doctype": "Chat Push Token",
			"fcm_token": fcm_token,
...
def unsubscribe(fcm_token: str) -> None:
	...
	frappe.db.delete("Chat Push Token", {"fcm_token": fcm_token, "user": frappe.session.user})
