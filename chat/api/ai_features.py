...
from chat.ai.handler import get_variables_for_instructions
...
def get_saved_prompts(bot: str = None):
	...
	prompts = frappe.get_list(
		"Chat Bot AI Prompt", or_filters=or_filters, fields=["name", "prompt", "is_global", "chat_bot"]
	)

	# Order by ones with the given bot
	prompts = sorted(prompts, key=lambda x: x.get("chat_bot") == bot, reverse=True)
