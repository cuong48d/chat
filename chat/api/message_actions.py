...
def get_action_defaults(action_id: str, message_id: str):
	...
	frappe.has_permission(doctype="Chat Message", doc=message_id, ptype="read", throw=True)
	action = frappe.get_doc("Chat Message Action", action_id)
	...
	channel_doc = frappe.get_doc("Chat Channel", message.channel_id)
	workspace_id = channel_doc.workspace

	if not workspace_id:
		# Get the last workspace that this user has access to
		workspace_id = frappe.get_last_doc("Chat Workspace", {"user_id": frappe.session.user}).name

	url = frappe.utils.get_url(f"/chat/{workspace_id}")
