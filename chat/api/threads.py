...
from chat.api.chat_channel import get_peer_user_id
from chat.utils import get_channel_members, get_thread_reply_count
...
def get_all_threads(
	...
	message = frappe.qb.DocType("Chat Message").as_("message")
	thread_messages = frappe.qb.DocType("Chat Message").as_("thread_messages")
...
def get_other_threads(
	...
	channel_member = frappe.qb.DocType("Chat Channel Member")
	thread_channel = frappe.qb.DocType("Chat Channel").as_("thread_channel")
	main_thread_message = frappe.qb.DocType("Chat Message").as_("main_thread_message")
	thread_channel_member = frappe.qb.DocType("Chat Channel Member").as_("thread_channel_member")
	thread_message = frappe.qb.DocType("Chat Message").as_("thread_message")
...
def create_thread(message_id):
	...
	thread_message = frappe.get_doc("Chat Message", message_id)

	channel_doc = frappe.get_cached_doc("Chat Channel", thread_message.channel_id)
	...
	if creator != frappe.session.user:
		frappe.get_doc(
			{"doctype": "Chat Channel Member", "channel_id": thread_channel.name, "user_id": creator}
