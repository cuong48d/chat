...
from chat.api.chat_channel import create_direct_message_channel, get_peer_user_id
from chat.utils import get_channel_member, is_channel_member, track_channel_visit
...
def fetch_recent_files(channel_id):
	...
	"""
	if not frappe.has_permission("Chat Channel", doc=channel_id):
...
def save_message(message_id, add=False):
	...
	toggle_like("Chat Message", message_id, add)

	liked_by = frappe.db.get_value("Chat Message", message_id, "_liked_by")
...
def get_pinned_messages(channel_id):
	...
	# check if the user has permission to view the channel
	frappe.has_permission("Chat Channel", doc=channel_id, ptype="read", throw=True)

	pinnedMessagesString = frappe.db.get_value("Chat Channel", channel_id, "pinned_messages_string")
...
def get_saved_messages():
	...
	query = (
		...
		.select(
			chat_message.name,
			chat_message.owner,
			chat_message.creation,
			chat_message.text,
			chat_message.channel_id,
			chat_message.file,
			chat_message.message_type,
			chat_message.message_reactions,
			chat_message._liked_by,
			chat_channel.workspace,
		)
		.where(chat_message._liked_by.like("%" + frappe.session.user + "%"))
		.where(
			(chat_channel.type.isin(["Open", "Public"]))
			| (chat_channel_member.user_id == frappe.session.user)
		)
...
def check_permission(channel_id):
	if frappe.get_cached_value("Chat Channel", channel_id, "type") == "Private":
...
def get_unread_count_for_channels():
	...
	channel = frappe.qb.DocType("Chat Channel")
	channel_member = frappe.qb.DocType("Chat Channel Member")
	message = frappe.qb.DocType("Chat Message")
	query = (
...
def get_unread_count_for_channel(channel_id):
	...
	if channel_member:
		last_timestamp = frappe.get_cached_value(
			"Chat Channel Member", channel_member["name"], "last_visit"
		)
		...
		if frappe.get_cached_value("Chat Channel", channel_id, "type") == "Open":
			return frappe.db.count(
				"Chat Message",
				filters={
					"channel_id": channel_id,
