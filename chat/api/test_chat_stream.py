...
from chat.api.chat_stream import get_messages, get_newer_messages, get_older_messages

CHANNEL_ID = "Public Workspace-test-channel"

EXTRA_TEST_RECORD_DEPENDENCIES = ["Chat Workspace"]
...
class TestChatStream(IntegrationTestCase):
	def setUp(self):
		try:
			frappe.delete_doc("Chat Channel", CHANNEL_ID)
	...
	def test_get_messages_around_base_mid(self):
		...
		base_message_id = frappe.db.get_value(
			"Chat Message", {"text": "Test Message 50", "channel_id": CHANNEL_ID}, "name"
		)
	...
	def test_get_messages_around_base_with_fewer_new_messages(self):
		...
		"""
		base_message_id = frappe.db.get_value(
			"Chat Message", {"text": "Test Message 95", "channel_id": CHANNEL_ID}, "name"
		)
		response = get_messages(CHANNEL_ID, base_message=base_message_id)
	...
	def test_get_messages_around_base_with_fewer_old_messages(self):
		...
		"""
		base_message_id = frappe.db.get_value(
			"Chat Message", {"text": "Test Message 5", "channel_id": CHANNEL_ID}, "name"
		)
		response = get_messages(CHANNEL_ID, base_message=base_message_id)
