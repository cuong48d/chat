...
def get_current_chat_user():
	"""
	Fetches the current user's Chat User profile
	"""

	# Check if the user is a Chat User and has he "Chat User" role
	# If not, then throw an error
	if not frappe.has_permission("Chat User"):
		frappe.throw(
			_(
				"You do not have a <b>Chat User</b> role. Please contact your administrator to add your user profile as a <b>Chat User</b>."
			),
			title=_("Insufficient permissions. Please contact your administrator."),
		)

	return frappe.get_cached_doc("Chat User", {"user": frappe.session.user})


@frappe.whitelist(methods=["POST"])
def update_chat_user(**args):
	"""
	Updates the current user's Chat User profile
	"""

	frappe.get_doc("Chat User", {"user": frappe.session.user}).update(args).save()
...
def get_list():
	"""
	Fetches list of all users who have the role: Chat User
	"""
...
def add_users_to_chat(users):
	...
	for user in users:
		...
		if user_doc.role_profile_name:
			...
			user_doc.append("roles", {"role": "Chat User"})
			user_doc.save()
...
def invite_user(email: str, first_name: str = None, last_name: str = None):
	"""
	Invites a user to Chat. If the user exists in Frappe, they are added to Chat.
	"""
	...
	if existing_user:
		...
		if user_doc.role_profile_name:
			frappe.throw(_("User has a role profile set. Please set the role to Chat User manually."))
