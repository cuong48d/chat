...
FORBIDDEN_DOCUMENT_TYPES = frozenset("Chat Message")


class ChatDocumentNotification(Document):
	...
	if TYPE_CHECKING:
		...
		from chat.chat_integrations.doctype.chat_document_notification_recipients.chat_document_notification_recipients import (
			ChatDocumentNotificationRecipients,
		)
		...
		recipients: DF.Table[ChatDocumentNotificationRecipients]
	...
	def validate(self):
		...
		frappe.cache().hdel("chat_doc_notifications", self.document_type)
		...
		for recipient in self.recipients:
			if recipient.channel_type == "User":
				if recipient.variable_type == "Static":
					# Check if this user exists in Chat
					if not frappe.db.exists("Chat User", recipient.value):
						frappe.throw(_("User {0} does not exist in Chat.").format(recipient.value))

			elif recipient.channel_type == "Channel":
				if recipient.variable_type == "Static":
					# Check if this channel exists in Chat
					if not frappe.db.exists("Chat Channel", recipient.value):
						frappe.throw(_("Channel {0} does not exist in Chat.").format(recipient.value))
	...
	def send_notification(self, context, link_doctype, link_document):
		...
		bot = frappe.get_doc("Chat Bot", self.sender)
...
def run_document_notification(doc, method):
	...
	Enqueues the notifications to be sent on Chat
	"""
	...
	chat_notifications = frappe.cache().hget(
		"chat_doc_notifications", doc.doctype, _get_notifications
	)
...
def evaluate_condition(context, condition, notification_name):
	...
	try:
		...
		message = _("Error while evaluating Chat Notification {0}. Please fix your template.").format(
			frappe.utils.get_link_to_form("Chat Document Notification", notification_name)
		)
		frappe.throw(message, title=_("Error in Chat Notification"))
		...
		frappe.log_error(
			title=title,
			message=message,
			reference_doctype="Chat Document Notification",
			reference_name=notification_name,
		)
		msg = f"<details><summary>{title}</summary>{message}</details>"
		frappe.throw(msg, title=_("Error in Chat Notification"))


def send_chat_notifications(doc, notifications_to_send, link_doctype, link_document):
	"""
	Background job to send notifications to Chat
	"""
	context = get_context(doc)
	for notification in notifications_to_send:
		notification_doc = frappe.get_doc("Chat Document Notification", notification.name)
