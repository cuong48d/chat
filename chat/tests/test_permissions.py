...
from chat.api.chat_channel import create_direct_message_channel
from chat.api.threads import create_thread

EXTRA_TEST_RECORD_DEPENDENCIES = ["User", "Chat User"]


class TestPermissions(IntegrationTestCase):
	def setUp(self):

		# test is a Chat Admin, test1 and test 3 are Chat Users, test2 is neither

		user = frappe.get_doc("User", "test@example.com")
		user.add_roles("Chat Admin")
		user.add_roles("Chat User")

		user1 = frappe.get_doc("User", "test1@example.com")
		user1.add_roles("Chat User")
		user1.remove_roles("Chat Admin")

		user2 = frappe.get_doc("User", "test2@example.com")
		user2.remove_roles("Chat Admin")
		user2.remove_roles("Chat User")

		user3 = frappe.get_doc("User", "test3@example.com")
		user3.add_roles("Chat User")
	...
	def test_workspace_create_permissions(self):
		...
		# CREATE: Chat Admin can create a workspace, but other users cannot
	...
	def test_workspace_read_permissions(self):
		# READ: Public workspaces are readable by all Chat Users, private workspaces are readable only by members
		...
		# Test2 is not a Chat User, so they cannot read the workspace
		frappe.set_user("test2@example.com")
		self.assertFalse(frappe.has_permission(test_workspace, ptype="read"))

	def test_workspace_member_create_permissions(self):
		"""
		If it's a public workspace, any Chat User can join the workspace
		...
		# It's a public workspace, so any Chat User can join the workspace
		...
		# Test2 should not be able to join the workspace since it's not a Chat User
		frappe.set_user("test2@example.com")
		with self.assertRaises(frappe.PermissionError):
			frappe.get_doc(
				{
		...
		self.assertEqual(
			2, frappe.db.count("Chat Workspace Member", {"workspace": test_workspace.name})
		)
		...
		self.assertEqual(
			3, frappe.db.count("Chat Workspace Member", {"workspace": test_workspace.name})
		)
	...
	def test_workspace_member_delete_permissions(self):
		...
		self.assertEqual(
			1, frappe.db.count("Chat Workspace Member", {"workspace": test_workspace.name})
		)
	...
	def test_channel_create_permissions(self):
		...
		DM channels can be created by any Chat User
		...
		# test1 should be able to create a direct message channel with test3 since both are Chat Users
		dm_t1_t3 = create_direct_message_channel(user_id="test3@example.com")

		# test2 should not be able to create a direct message channel with test3 since test2 is not a Chat User
		frappe.set_user("test2@example.com")
		with self.assertRaises(frappe.PermissionError):
		...
		self.assertEqual(1, frappe.db.count("Chat Channel Member", {"channel_id": test_channel.name}))
	...
	def test_channel_delete_permissions(self):
		...
		self.assertFalse(frappe.db.exists("Chat Channel", test_channel.name))
		...
		frappe.set_user("test@example.com")
		with self.assertRaises(frappe.PermissionError):
			frappe.delete_doc("Chat Channel", dm_t1_t3)
		...
		with self.assertRaises(frappe.PermissionError):
			frappe.delete_doc("Chat Channel", test_message.name)
		...
		self.assertFalse(frappe.db.exists("Chat Channel", test_message.name))
	...
	def test_channel_read_permissions(self):
		...
		self.assertTrue(frappe.has_permission("Chat Channel", doc=public_channel.name))
		self.assertTrue(frappe.has_permission("Chat Channel", doc=open_channel.name))
		...
		frappe.set_user("test3@example.com")
		self.assertFalse(frappe.has_permission("Chat Channel", doc=public_channel.name))
		self.assertFalse(frappe.has_permission("Chat Channel", doc=open_channel.name))

		# Test should be able to read the private channel, test1 and test3 should not be able to read the private channel
		frappe.set_user("test@example.com")
		self.assertTrue(frappe.has_permission("Chat Channel", doc=private_channel.name))

		frappe.set_user("test1@example.com")
		self.assertFalse(frappe.has_permission("Chat Channel", doc=private_channel.name))
		...
		self.assertTrue(frappe.has_permission("Chat Channel", doc=test_message.name))

		# Test1 should not be able to read the thread since they are not a member of the channel
		frappe.set_user("test1@example.com")
		self.assertFalse(frappe.has_permission("Chat Channel", doc=test_message.name))
	...
	def test_channel_member_permissions(self):
		...
		self.assertTrue(
			frappe.db.exists(
				"Chat Channel Member", {"channel_id": open_channel.name, "user_id": "test1@example.com"}
			)
		)
		self.assertTrue(
			frappe.db.exists(
				"Chat Channel Member", {"channel_id": public_channel.name, "user_id": "test1@example.com"}
			)
		)
		...
		self.assertTrue(
			frappe.db.exists(
				"Chat Channel Member", {"channel_id": private_channel.name, "user_id": "test1@example.com"}
			)
		)
		...
		self.assertTrue(
			frappe.has_permission("Chat Channel Member", doc=test1_open_channel_member.name, ptype="write")
		)
		self.assertTrue(
			frappe.has_permission(
				"Chat Channel Member", doc=test1_public_channel_member.name, ptype="write"
			)
		)
		self.assertTrue(
			frappe.has_permission(
				"Chat Channel Member", doc=test1_private_channel_member.name, ptype="write"
			)
		)
		...
		self.assertTrue(
			frappe.db.get_value("Chat Channel Member", test1_open_channel_member.name, "is_admin")
		)
		self.assertTrue(
			frappe.db.get_value("Chat Channel Member", test1_private_channel_member.name, "is_admin")
		)
		...
		frappe.set_user("test1@example.com")
		self.assertTrue(frappe.has_permission("Chat Channel", doc=thread["thread_id"], ptype="read"))
	...
	def test_message_permissions(self):
		...
		self.assertTrue(frappe.has_permission("Chat Message", doc=test_message.name, ptype="read"))
		...
		self.assertFalse(frappe.db.exists("Chat Message", test_message.name))
		...
		self.assertFalse(frappe.db.exists("Chat Message", test1_message.name))
