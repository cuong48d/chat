...
def check_app_permission():
	...
	if frappe.db.exists("Chat User", {"user": frappe.session.user}):
		return True

	return False


def chat_user_has_permission(doc, user=None, ptype=None):

	if doc.type == "Bot":
		# Anyone with Chat User role can change the bot details
		if user != "Guest":
			return True
	else:
...
def workspace_has_permission(doc, user=None, ptype=None):
	"""
	1. Only Chat Admins can create a workspace
	...
	"""
	if ptype == "create":
		# Return True since this is checked via the Role in the doctype - only Chat Admins can create a workspace
		return True
...
def workspace_member_has_permission(doc, user=None, ptype=None):
	...
	if ptype == "create":
		workspace_type = frappe.get_cached_value("Chat Workspace", doc.workspace, "type")
...
def channel_has_permission(doc, user=None, ptype=None):
	if doc.is_direct_message:
		...
		if ptype == "create":
			...
			main_channel = frappe.get_cached_value("Chat Message", doc.channel_name, "channel_id")
		...
		if ptype == "create":
			...
			if not frappe.db.get_value("Chat Workspace", doc.workspace, "only_admins_can_create_channels"):
				return True
	...
	# 	if doc.workspace:
	# 		return frappe.db.exists("Chat Workspace Member", {"workspace": doc.workspace, "user": user})
	# 	return True
	# elif doc.type == "Private":
	# 	if doc.is_thread:
	# 		if ptype == "read" or ptype == "create":
	# 			# Only users who are part of the original channel can read the thread
	# 			return frappe.has_permission(doctype="Chat Message", doc=doc.name, ptype="read")

	# 	if frappe.db.exists("Chat Channel Member", {"channel_id": doc.name, "user_id": user}):
	# 		return True
	# 	elif (
	# 		doc.owner == user and frappe.db.count("Chat Channel Member", {"channel_id": doc.name}) <= 0
	# 	):
	# 		return True
	# 	else:
	# 		return False
...
def channel_member_has_permission(doc, user=None, ptype=None):
	...
	if ptype == "read":
		...
		return frappe.db.exists("Chat Channel Member", {"channel_id": doc.channel_id, "user_id": user})

	channel_doc = frappe.get_cached_doc("Chat Channel", doc.channel_id)
	...
	if ptype == "create":
		if is_direct_message:
			...
			if channel_doc.is_thread:
				# Check if the user is a member of the main channel
				main_channel = frappe.db.get_value("Chat Message", doc.channel_id, "channel_id")
				return is_channel_member(main_channel, user)
			...
			if is_workspace_member(workspace, doc.user_id):
				return frappe.db.exists(
					"Chat Channel Member", {"channel_id": doc.channel_id, "user_id": user}
				)
	...
	# 	if frappe.db.exists("Chat Channel Member", {"channel_id": doc.channel_id, "user_id": user}):
	# 		return True
...
def chat_poll_vote_has_permission(doc, user=None, ptype=None):
	"""
	...
	if not frappe.has_permission("Chat Poll", doc=doc.poll_id, ptype="read", user=user):
		return False

	if ptype == "create":
		# User can only vote if they are a member of the channel
		channel_id = frappe.get_cached_value("Chat Message", {"poll_id": doc.poll_id}, "channel_id")
	...
	if ptype in ["read", "delete"]:
		if doc.owner == user:
			...
			is_anonymous = frappe.get_cached_value("Chat Poll", doc.poll_id, "is_anonymous")
			if not is_anonymous:
				if ptype == "read":
					return True
...
def chat_poll_has_permission(doc, user=None, ptype=None):
	"""
	...
	if ptype == "read" or ptype == "delete" or ptype == "write":
		...
		message = frappe.get_doc("Chat Message", {"poll_id": doc.name})
...
def chat_channel_query(user):
	...
	"""
      ...
      We could also remove "Chat User" from the Chat Channel doctype role, but then permission checks for joining socket rooms for the channel would fail

      Hence, we are adding a WHERE clause to the query - this is inconsequential since we will never use the standard get_list query for Chat Channel,
      but needed for security since we do not want users to be able to view channels they are not a member of
    """
	return f"`tabChat Channel`.owner = {frappe.db.escape(user)}"


def chat_message_query(user):
	if not user:
		user = frappe.session.user
	...
	"""
      ...
      We could also remove "Chat User" from the Chat Message doctype role, but then permission checks for attached files would also fail.

      Hence, we are adding a WHERE clause to the query - this is inconsequential since we will never use the standard get_list query for Chat Message,
      but needed for security since we do not want users to be able to view messages from channels they are not a member of
    """
	return f"`tabChat Message`.owner = {frappe.db.escape(user)}"


def chat_poll_query(user):
	if not user:
		user = frappe.session.user

	return f"`tabChat Poll`.owner = {frappe.db.escape(user)}"


def chat_poll_vote_query(user):
	if not user:
		user = frappe.session.user
	...
	"""
	  ...
	  Hence, we are adding a WHERE clause to the query - this is inconsequential since we will never use the standard get_list query for Chat Poll Vote,
	  but needed for security since we do not want users to be able to view votes from polls they did not vote for
	"""
	return f"`tabChat Poll Vote`.owner = {frappe.db.escape(user)}"
