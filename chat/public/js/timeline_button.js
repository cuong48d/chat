      ...
      refresh: function (frm) {
        if (!frm.is_new()) {
          if (frm.footer?.frm?.timeline && !buttonAdded) {
            ...
            let get_channels = () => {
              return frappe.call({
                method: 'chat.api.chat_channel.get_channels',
                args: {
                  hide_archived: true,
                },
                callback: function (r) {
            ...
            // check the button is not already added by checking the class name 'send-chat-button'

            timeline.add_action_button(
              __('Send a Chat'),
              send_chat,
              'share',
              'btn-secondary send-chat-button'
            );
            buttonAdded = true;
          }
        }
