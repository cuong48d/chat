...
const SkeletonLoader = () => {
    ...
    return <>{Array.from({ length: 17 }).map((_, i) => <div key={i} className="chat-channel-list-item">
...
const ChannelListItem = ({ channel, onClick }) => {
    ...
    return <button className="chat-channel-list-item" onClick={onClick}>
        <div className='channel-label'>
            <span className='chat-channel-icon'>
                <ChannelIcon channelType={channel.channel_type} />
            </span>
            <span className="chat-channel-list-item__name">{channel.channel_name}</span>
        </div>
        <div>
...
const DMChannelListItem = ({ channel, onClick }) => {
    ...
    return <button className="chat-channel-list-item" onClick={onClick}>
        <div className='channel-label'>
            <Avatar user={user} />
            <span className="chat-channel-list-item__name">{user?.full_name}</span>
        </div>
        <div>
            {count ? <span className="channel-unread-count">{count}</span> : null}
        </div>
