...
const MessageStream = ({ channelID }) => {
  ...
  const { data } = useSWRSubscription(
    `chat.api.chat_message.get_messages_with_dates?channel_id=${channelID}`,
    (key, { next }) => {
      //Initial load
      fetcher(key).then((data) => next(null, data));

      if (channelID) {
        frappe.socketio.doc_subscribe("Chat Channel", channelID);
        frappe.socketio.doc_open("Chat Channel", channelID)
      ...
      return () => {
        ...
        frappe.socketio.doc_close("Chat Channel", channelID);
        try {
          frappe.socketio.doc_unsubscribe("Chat Channel", channelID);
        } catch (e) {
          // console.log(e)
        }
      }
  ...
  return (
    <div>
      ...
      <div className="chat-message-stream-container" ref={containerRef}>
