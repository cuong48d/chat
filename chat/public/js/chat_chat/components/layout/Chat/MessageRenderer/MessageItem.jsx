...
const MessageHeader = ({ message, user }) => {
    ...
    return <div className='chat-message-item-header'>
        <span className='chat-message-item-header-name'>{user?.full_name}</span>
        <span className='chat-message-item-header-time'>{moment(message.creation, frappe.defaultDatetimeFormat).format('HH:mm A')}</span>
    </div>
}
