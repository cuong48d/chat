...
class ChatAIFunction(Document):
	...
	if TYPE_CHECKING:
		...
		from chat.chat_ai.doctype.chat_ai_function_params.chat_ai_function_params import (
			ChatAIFunctionParams,
		)
		...
		parameters: DF.Table[ChatAIFunctionParams]
		params: DF.JSON | None
	...
	def on_update(self):
		...
		bots = frappe.get_all("Chat Bot Functions", filters={"function": self.name}, pluck="parent")

		for bot in bots:
			bot = frappe.get_doc("Chat Bot", bot)
