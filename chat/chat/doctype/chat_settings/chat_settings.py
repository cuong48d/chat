...
class ChatSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from chat.chat_integrations.doctype.chat_hr_company_workspace.chat_hr_company_workspace import (
			ChatHRCompanyWorkspace,
		)

		auto_add_system_users: DF.Check
		auto_create_department_channel: DF.Check
		company_workspace_mapping: DF.Table[ChatHRCompanyWorkspace]
		...
		push_notification_service: DF.Literal["Frappe Cloud", "Chat"]
		show_if_a_user_is_on_leave: DF.Check
		show_chat_on_desk: DF.Check
