...
from chat.api.notification import are_push_notifications_enabled


class ChatPushToken(Document):
	...
	def on_trash(self):
		...
		if push_service == "Frappe Cloud" and are_push_notifications_enabled():
			try:
				...
				unsubscribe(self.fcm_token, "chat")
			except ImportError:
				# push notifications are not supported in the current framework version
				pass
			except Exception:
	...
	def get_push_service(self) -> str:
		...
		"""
		push_service = frappe.db.get_single_value("Chat Settings", "push_notification_service")
