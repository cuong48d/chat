...
class ChatWorkspace(Document):
	...
	def on_trash(self):
		...
		frappe.db.delete("Chat Workspace Member", {"workspace": self.name})
		# Delete all channels when the workspace is deleted
		channels = frappe.db.get_all("Chat Channel", {"workspace": self.name})
		for channel in channels:
			# use delete doc to delete the channel ,it's members and messages
			frappe.delete_doc("Chat Channel", channel.name)

	def create_member_for_owner(self):
		member = frappe.new_doc("Chat Workspace Member")
