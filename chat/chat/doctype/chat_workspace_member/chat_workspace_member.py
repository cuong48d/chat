...
class ChatWorkspaceMember(Document):
	...
	def validate(self):
		...
		if frappe.db.exists("Chat Workspace Member", conditions):
	...
	def delete_channel_members_for_user(self):
		...
		"""
		workspace_channels = frappe.get_all("Chat Channel", {"workspace": self.workspace}, pluck="name")
		for channel in workspace_channels:
			frappe.db.delete("Chat Channel Member", {"channel_id": channel, "user_id": self.user})
...
def on_doctype_update():
	"""
	Add indexes to Chat Workspace Member table
	"""
	# Index the selector first for faster queries (less rows to sort in the next step)
	frappe.db.add_index("Chat Workspace Member", ["workspace", "user"])
