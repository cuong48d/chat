...
const useUnreadMessageCount = () => {
    ...
    const { data: unread_count, mutate: updateCount } = useFrappeGetCall<{ message: UnreadCountData }>("chat.api.chat_message.get_unread_count_for_channels",
        undefined,
 ...
 * Hook to manage unread message count state with realtime events. This is only used in the sidebar.
    ...
    For every channel member, we store a last_visit timestamp in the Chat Channel Member doctype. To get the number of unread messages, we can simply look at the no. of messages created after this timestamp for any given channel.
    ...
    3. When the user sends a new message (handled by controller under Chat Message)
    4. When the user exits a channel after all the latest messages had been loaded (called from the frontend)

    When Chat loads, we fetch the unread message counts for all channels. Post that, updates to these counts are made when:
...
export const useFetchUnreadMessageCount = () => {
    ...
    const fetchUnreadCountForChannel = async (channelID: string) => {
        ...
        updateCount(d => {
            if (d) {
                return call.get('chat.api.chat_message.get_unread_count_for_channel', {
    ...
    useFrappeEventListener('chat:unread_channel_count_updated', (event) => {
        ...
        if (event.sent_by !== currentUser) {
            ...
            if (channelID === event.channel_id && !state?.baseMessage) {
                ...
                // https://github.com/The-Commit-Company/Chat/pull/745#issuecomment-2014313429
    ...
    useEffect(() => {
        // @ts-expect-error
        const app_name = window.app_name || "Chat"
        if (unread_count) {
