import { ChatDocumentNotificationRecipients } from './ChatDocumentNotificationRecipients'

export interface ChatDocumentNotification{
	creation: string
	name: string
	modified: string
	owner: string
	modified_by: string
	docstatus: 0 | 1 | 2
	parent?: string
	parentfield?: string
	parenttype?: string
	idx?: number
	/**	Notification Name : Data	*/
	notification_name: string
	/**	Enabled : Check	*/
	enabled?: 0 | 1
	/**	Sender : Link - Chat Bot	*/
	sender: string
	/**	Send Alert On : Select	*/
	send_alert_on: "New Document" | "Update" | "Submit" | "Cancel" | "Delete"
	/**	Document Type : Link - DocType	*/
	document_type: string
	/**	Condition : Code - Optional: The alert will be sent if this expression is true	*/
	condition?: string
	/**	Recipients : Table - Chat Document Notification Recipients	*/
	recipients: ChatDocumentNotificationRecipients[]
	/**	Message : Code - Can be HTML/Markdown/Plain Text. Support Jinja tags	*/
	message: string
}