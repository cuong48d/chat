    ...
    return (
        <Container>
            <Flex align='center' justify='center' className='h-screen'>
                {canAddChatUsers ? <AddChatUsersCard /> : <ChatUsersAlert />}
            </Flex>
        </Container>
    )
}
            ...
            <Flex direction='column' align='center' gap='4'>
                ...
                <Flex direction='column' align='center' gap='2'>
                    <Heading as='h2' className='not-cal' size='4'>
                        You do not have access to <Text className='cal-sans'>Chat</Text>.
                    </Heading>
                    <Text as='p' size='2' align='center' className='leading-6'>
                        Please contact your Administrator or System Manager to give you the <Strong>"Chat User"</Strong> role.
                    </Text>
                    <Flex gap='3' direction='column' py='1' pt='3'>
                        <Button asChild>
                            <Link className='text-white' href={'/app/chat-user'}>View Chat Users</Link>
                        </Button>
...
const AddChatUsersCard = () => {
    ...
    const { loading, call, error: postError } = useFrappePostCall('chat.api.chat_users.add_users_to_chat')

    const handleAddUsers = () => {
        call({
            users: JSON.stringify([...selected, currentUser])
        }).then(() => {
            toast.success(`You have added ${selected.length} users to Chat`)
    ...
    return <Box className={clsx(BOX_STYLE, 'bg-accent-a2')}>
        <Card className={clsx(CARD_STYLE, 'max-w-xl min-w-[720px]')}>
            <Flex gap='2' direction='column'>
                <Heading as='h1'>Add users to Chat</Heading>
                <Text as='p' size='2' className='leading-6'>
                    Users you add will be given the <Strong>"Chat User"</Strong> role.
                </Text>
            </Flex>
...
export default AddChatUsersPage